...
class KMeans:
    ...
    def fit(self, X): 
       ...
       # centroids = np.random.uniform(np.amin(X,axis=0),np.amax(X,axis=0),size=(self.k, X.shape[1]))
        ...
        #if(self.centroids.shape[0] != 8):
         #   self.centroids = np.append(self.centroids, np.random.uniform(np.amin(X, axis= 0), np.amax(X,axis = 0), size = (-centroids.shape[0]+ self.k, X.shape[1])), axis = 0)
    ...
    def predict(self, X):
        ...
        for i in range(self.maxIter):
            temp = []
            for point in X:
                distance = euclidean_distance(point, self.centroids)
            ...
            for j,index in enumerate(cluster_index):
                if(len(index) == 0):
                    cluster_center.append(self.centroids[j])
                else:
